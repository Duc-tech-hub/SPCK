...
(async () => {
    ...
    const protectedPages = ["/search.html", "/userinfo.html"];
            ...
            signInWithPopup(auth, googleProvider)
                .then(async (result) => {
                    ...
                    try {
                        ...
                        console.log("Đã lưu lịch sử đăng nhập.");
                        window.location.href = "../html/home.html"
                    } catch (e) {
                        console.error("Lỗi khi tạo history:", e);
                    }
                })
                .catch((error) => {
...
onAuthStateChanged(auth, (user) => {
    if (user) {
        ...
        if (path.includes("index.html") && isLoggingIn === false ||
            path === "/" ||
            path === "") {
